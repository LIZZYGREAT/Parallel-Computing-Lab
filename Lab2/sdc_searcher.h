...
private:
    ...
    void scan_probe_list(int list_id, float coarse_dist, const float* query,
        ...
        sdc_quantize_and_build_lut(ws.residual, FS_M, index->d_sub,
                                   index->pq_centroids.data(), center_dist_table.data(),
                                   ws.lut_f, ws.query_code,
                                   min_val, max_val);
...
public:
    SDCSearcher(const IVFPQIndex* idx, const float* base, int ratio = 20)
        ...
        tp::parallel_for_static(0, static_cast<size_t>(FS_M * FS_K * FS_K), [&](size_t flat) {
            int m = static_cast<int>(flat / (FS_K * FS_K));
            int rem = static_cast<int>(flat % (FS_K * FS_K));
            int i = rem / FS_K;
            int j = rem % FS_K;
