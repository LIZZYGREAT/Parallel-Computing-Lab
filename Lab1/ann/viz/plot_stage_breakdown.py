...
def plot_stage_breakdown(profiler=None, threads=4, out_dir=None):
    ...
    if df.empty:
        print("[Error] 无 profiler_detail_* 数据")
        return
    ...
    if adc:
        _draw_stacked(axes[0, 0], adc, f"ADC 搜索阶段占比 (T={threads})")
    ...
    if sdc:
        _draw_stacked(axes[0, 1], sdc, f"SDC 搜索阶段占比 (T={threads})")
    ...
    for t in sorted(df["Threads"].unique()):
        ...
        for method, stages, scan_key in [
            ("ADC", ADC_STAGES, "5_ADC_Scan"),
            ("SDC", SDC_STAGES, "5_SDC_Scan"),
        ]:
            d = _pct_table(df, method, t, stages)
            if d is None:
                continue
            si = stages.index(scan_key)
            ax.plot(d["nprobes"], d["mat"][:, si], "o-", linewidth=2, label=f"{method} 扫描")
        ax.set_xlabel("NProbe")
        ax.set_ylabel("扫描阶段占比 (%)")
        ax.set_title(f"扫描阶段占比 vs NProbe (T={t})")
